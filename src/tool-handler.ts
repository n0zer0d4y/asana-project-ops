...
import { enabledToolCategories, enabledTools, combineEnabledTools } from './index.js';
  ...
  return async (request: CallToolRequest) => {
    ...
    try {
      ...
      if (enabledToolCategories.length > 0 || enabledTools.length > 0) {
        ...
        if (!combinedEnabledTools.includes(request.params.name)) {
          throw new Error(
            `Tool '${request.params.name}' is not enabled. Enabled tools: ${combinedEnabledTools.join(
              ", "
            )}`
          );
        }
      ...
      switch (request.params.name) {
        ...
        case "asana_batch_create_tasks": {
          const { project_id, tasks, continue_on_error = false } = args;
          try {
            const response = await asanaClient.batchCreateTasks(project_id, tasks, continue_on_error);
            return {
              content: [{ type: "text", text: JSON.stringify(response) }],
            };
          } catch (error) {
            // When error occurs and html_notes was provided in any task, validate it
            const tasksWithHtmlNotes = tasks.filter((task: any) => task.html_notes);
            if (tasksWithHtmlNotes.length > 0 && error instanceof Error && [400, 500].includes(error.status)) {
              const validationErrors = [];
              for (let i = 0; i < tasksWithHtmlNotes.length; i++) {
                const task = tasksWithHtmlNotes[i];
                const xmlValidationErrors = validateAsanaXml(task.html_notes);
                if (xmlValidationErrors.length > 0) {
                  validationErrors.push({
                    task_index: tasks.indexOf(task),
                    validation_errors: xmlValidationErrors
                  });
                }
              }
                ...
                return {
                  content: [{
                    ...
                    text: JSON.stringify({
                      ...
                      message: "Some tasks contain invalid HTML notes formatting. Please check the validation errors above."
                    })
                  }],
                };
              }
        ...
        case "asana_batch_update_tasks": {
          const { task_updates, continue_on_error = false } = args;
          try {
            const response = await asanaClient.batchUpdateTasks(task_updates, continue_on_error);
            return {
              content: [{ type: "text", text: JSON.stringify(response) }],
            };
          } catch (error) {
            // When error occurs and html_notes was provided in any task update, validate it
            const updatesWithHtmlNotes = task_updates.filter((update: any) => update.html_notes);
            if (updatesWithHtmlNotes.length > 0 && error instanceof Error && error.message.includes('400')) {
              const validationErrors = [];
              for (let i = 0; i < updatesWithHtmlNotes.length; i++) {
                const update = updatesWithHtmlNotes[i];
                const xmlValidationErrors = validateAsanaXml(update.html_notes);
                if (xmlValidationErrors.length > 0) {
                  validationErrors.push({
                    task_index: task_updates.indexOf(update),
                    validation_errors: xmlValidationErrors
                  });
                }
              }
                ...
                return {
                  content: [{
                    ...
                    text: JSON.stringify({
                      error: error instanceof Error ? error.message : String(error),
                      validation_errors: validationErrors,
                      message: "Some task updates contain invalid HTML notes formatting. Please check the validation errors above."
                    })
                  }],
                };
              } else {
        ...
        case "asana_batch_create_subtasks": {
          const { subtasks, continue_on_error = false } = args;
          try {
            const response = await asanaClient.batchCreateSubtasks(subtasks, continue_on_error);
            return {
              content: [{ type: "text", text: JSON.stringify(response) }],
            };
          } catch (error) {
            // When error occurs and html_notes was provided in any subtask, validate it
            const subtasksWithHtmlNotes = subtasks.filter((subtask: any) => subtask.html_notes);
            if (subtasksWithHtmlNotes.length > 0 && error instanceof Error && [400, 500].includes(error.status)) {
              const validationErrors = [];
              for (let i = 0; i < subtasksWithHtmlNotes.length; i++) {
                const subtask = subtasksWithHtmlNotes[i];
                const xmlValidationErrors = validateAsanaXml(subtask.html_notes);
                if (xmlValidationErrors.length > 0) {
                  validationErrors.push({
                    subtask_index: subtasks.indexOf(subtask),
                    validation_errors: xmlValidationErrors
                  });
                }
              }
                ...
                return {
                  content: [{
                    ...
                    text: JSON.stringify({
                      error: error instanceof Error ? error.message : String(error),
                      validation_errors: validationErrors,
                      message: "Some subtasks contain invalid HTML notes formatting. Please check the validation errors above."
                    })
                  }],
                };
              }
        ...
        case "asana_batch_delete_tasks": {
          const { task_ids, continue_on_error = false } = args;
          const response = await asanaClient.batchDeleteTasks(task_ids, continue_on_error);
          return {
            content: [{ type: "text", text: JSON.stringify(response) }],
          };
        }

        case "asana_create_task_with_subtasks": {
          const { project_id, subtasks, continue_on_error = false, ...taskData } = args;
          try {
            const response = await asanaClient.createTaskWithSubtasks(
              project_id,
              taskData,
              subtasks, // Now optional
              continue_on_error
            );
            return {
              content: [{ type: "text", text: JSON.stringify(response) }],
            ...
            // When error occurs and html_notes was provided, validate it
            const taskWithHtmlNotes = taskData.html_notes;
            if (taskWithHtmlNotes && error instanceof Error && [400, 500].includes(error.status)) {
              const xmlValidationErrors = validateAsanaXml(taskWithHtmlNotes);
              if (xmlValidationErrors.length > 0) {
                return {
                  content: [{
                    type: "text",
                    text: JSON.stringify({
                      error: error instanceof Error ? error.message : String(error),
                      validation_errors: xmlValidationErrors,
                      message: "The task contains invalid HTML notes formatting. Please check the validation errors above."
                    })
                  }],
                };
              }
            }
            // When error occurs and html_notes was provided in subtasks, validate them
        ...
        case "asana_batch_create_tasks_with_subtasks": {
          const { project_id, tasks, continue_on_error = false } = args;
          try {
            const response = await asanaClient.batchCreateTasksWithSubtasks(
              project_id,
              tasks,
              continue_on_error
            );
            ...
            // When error occurs and html_notes was provided in any task or subtask, validate them
            const tasksWithHtmlNotes = tasks.filter((task: any) => task.html_notes);
            const allSubtasksWithHtmlNotes = tasks.flatMap((task: any) =>
              task.subtasks ? task.subtasks.filter((subtask: any) => subtask.html_notes) : []
            );

            if ((tasksWithHtmlNotes.length > 0 || allSubtasksWithHtmlNotes.length > 0) &&
                error instanceof Error && [400, 500].includes(error.status)) {
              const validationErrors = [];

              // Validate task HTML notes
              for (let i = 0; i < tasksWithHtmlNotes.length; i++) {
                const task = tasksWithHtmlNotes[i];
                const xmlValidationErrors = validateAsanaXml(task.html_notes);
                if (xmlValidationErrors.length > 0) {
              ...
              // Validate subtask HTML notes
              for (let i = 0; i < allSubtasksWithHtmlNotes.length; i++) {
                const subtask = allSubtasksWithHtmlNotes[i];
                const xmlValidationErrors = validateAsanaXml(subtask.html_notes);
                if (xmlValidationErrors.length > 0) {
                  // Find which task this subtask belongs to
                  const taskIndex = tasks.findIndex((task: any) =>
                    task.subtasks && task.subtasks.includes(subtask)
                  );
                  const subtaskIndex = tasks[taskIndex].subtasks.indexOf(subtask);
                ...
                return {
                  content: [{
                    ...
                    text: JSON.stringify({
                      error: error instanceof Error ? error.message : String(error),
                      validation_errors: validationErrors,
                      message: "Some tasks or subtasks contain invalid HTML notes formatting. Please check the validation errors above."
                    })
                  }],
                };
              }
        ...
        case "asana_section_operations": {
          const { operations, continue_on_error = false } = args;
          try {
            const response = await asanaClient.sectionOperations(operations, continue_on_error);
            return {
              content: [{ type: "text", text: JSON.stringify(response) }],
            };
          } catch (error) {
