...
When adding a new tool, ensure proper organization and registration.
...
tasksApiInstance.getTasks({ limit: 50 }).then((firstPage) => {
...
1. **Define the tool** in `src/tools/<category>-tools.ts`:
   ...
   - Add HTML validation for applicable text parameters
...
2. **Add API method** in `src/asana-client-wrapper.ts`:
   ...
   - Support batch operations with `continue_on_error` where applicable
...
6. **Test the tool**:
   - Build: `npm run build`
   - Test using MCP Inspector with various parameter combinations
